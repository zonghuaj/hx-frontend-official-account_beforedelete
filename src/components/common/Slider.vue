<template>
  <div id="puzzle" ref="puzzle" style="display:inline-block;margin-top: 30px;">
    <div class="puzzle-container" :style="'border-radius:'+16*scale+'px;'">
      ...
      <div class="re-btn" @click="refreshImg" :style="'height: '+28*scale+'px;font-size: '+18*scale+'px;'">
        <!--<a-icon type="redo"/>-->
    ...
    <div class="slider-container" :style="'width:' + dataWidth + 'px;'">
      ...
      <div class="slider-btn" ref="sliderBtn" @mousedown="startMove" @touchstart="startMove">
        <!--<a-icon type="pause-circle" theme="twoTone" twoToneColor="#52c41a" :style="'font-size: '+44*scale+'px;'" />-->

        <p style="width: 40px;height:28px;background: #ff8500;color:white"><van-icon name="arrow" color="white" style="margin-top:6px"/></p>
      </div>
    </div>
  </div>
</template>
...
<style scoped>
  .slider-btn{position:absolute;width:44px;height:44px;left:0;top:1px;z-index:12;cursor:pointer;background-position:0 -84px;transition:inherit}
  ...
  .hidden{display:none}
  .re-btn{position:absolute;right:0;top:0;height:28px;padding:0 16px;z-index: 100}
  ...
  .puzzle-container{position:relative;border:1px solid #ddd;}
  .slider-container{position:relative;margin:auto;}
  .slider-bar{border:1px solid #e8e8e8;background:#fff;box-shadow:0 1px 1px rgba(12,10,10,0.2) inset;}
  .slider-bar-text{font-size:12px;color: #ccc;line-height:28px;margin:0;text-align:right;padding-right:22px;}
